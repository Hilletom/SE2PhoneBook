...
public class EntryArea {
    ...
    public EntryArea(PhoneBook phoneBook, SearchArea searchArea) {
        ...
        lastNameCol.setOnEditCommit(t -> {
            getCurrentRow(t).setLastName(t.getNewValue());
        ...
        firstNameCol.setOnEditCommit(t -> {
            getCurrentRow(t).setFirstName(t.getNewValue());
            phoneBook.addEmptyEntry();
        });
        ...
        emailCol.setOnEditCommit(t -> {
            getCurrentRow(t).setNumber(t.getNewValue());
            phoneBook.addEmptyEntry();
        });
    ...
    public void setItems(List<TelefonEntry> items) {
        if (items.isEmpty() || !items.get(items.size() - 1).isNew())
            items.add(new TelefonEntry()); // TODO: DELETE
